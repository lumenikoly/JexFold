...
function runEmcmake(args) {
  ...
  return run(path.join(process.env.EMSDK, 'upstream', 'emscripten', 'emcmake.bat'), args, {
    shell: true,
  });
}
...
try {
  ...
  if (!commit.startsWith(EXPECTED_COMMIT_PREFIX))
    throw new Error(`Unexpected libjxl checkout: ${commit}.`);
  run(
    'git',
    [
      'submodule',
  ...
  runEmcmake([
    'cmake',
    '-S',
    cmakePath(path.join(root, 'wasm')),
    '-B',
    cmakePath(build),
    '-DCMAKE_BUILD_TYPE=Release',
    `-DLIBJXL_SOURCE_DIR=${cmakePath(source)}`,
  ]);
  run('cmake', [
    '--build',
    cmakePath(build),
    ...
    'jexfold_codec',
    '--parallel',
    String(Math.min(4, os.availableParallelism())),
  ]);
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ libjxlVersion: VERSION, commit, simd: true, threads: false }, null, 2) + '\n',
  );
