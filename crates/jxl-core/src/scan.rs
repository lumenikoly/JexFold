...
fn is_source(path: &Path, mode: ConversionMode) -> bool {
    match mode {
        ...
        ConversionMode::JxlToJpeg => path
            .extension()
            .and_then(|s| s.to_str())
            .is_some_and(|s| s.eq_ignore_ascii_case("jxl")),
    }
}
...
pub fn scan_sources(
    ...
    if inputs.is_empty() {
        return Err(Error::Invalid("Выберите файлы или папку".into()));
    }
    ...
    for input in inputs {
        ...
        if fs::symlink_metadata(input)?.file_type().is_symlink() {
            return Err(Error::Invalid(format!(
                "Символическая ссылка не поддерживается: {}",
                input.display()
            )));
        }
        ...
        if path.to_str().is_none() {
            return Err(Error::Invalid(
                "Имена файлов должны быть представимы в UTF-8".into(),
            ));
        }
    ...
    for root in roots {
        ...
        let parent = root
            .parent()
            .ok_or_else(|| Error::Invalid("Выберите папку, а не корень диска".into()))?;
        ...
        let label = labels
            ...
            .clone();
        if root.is_file() {
            let relative = PathBuf::from(&label).join(
                root.file_name()
                    .ok_or_else(|| Error::Invalid("Некорректное имя".into()))?,
            );
        ...
        if !root.is_dir() {
            warning(
                &mut result,
                format!("Необычный тип файла: {}", root.display()),
            );
            continue;
        }
        for entry in WalkDir::new(&root).follow_links(false).sort_by_file_name() {
            ...
            match entry {
                Ok(entry) if entry.file_type().is_symlink() => {
                    warning(
                        &mut result,
                        format!("Ссылка пропущена: {}", entry.path().display()),
                    );
                }
                Ok(entry) if entry.file_type().is_file() && is_source(entry.path(), mode) => {
                    ...
                    if let Err(error) =
                        add_file(entry.path(), relative, mode, &mut result, &mut seen)
                    {
    ...
    if result.files.is_empty() {
        return Err(Error::Invalid(format!(
            "В выбранных источниках нет доступных {}",
            if mode == ConversionMode::JpegToJxl {
                "JPEG"
            } else {
                "JXL"
            }
...
fn add_file(
    ...
    if result.files.len() >= MAX_FILES {
        return Err(Error::Invalid(format!(
            "В одной очереди допускается до {MAX_FILES} файлов. Выберите меньшую папку."
        )));
    }
...
mod tests {
    ...
    fn cancellation_stops_scanning() {
        ...
        c.cancel();
        assert!(matches!(
            scan_sources(&[PathBuf::from("/tmp")], ConversionMode::JpegToJxl, &c),
            Err(Error::Cancelled)
        ));
    }
    #[cfg(unix)]
    ...
    fn separately_selected_files_keep_one_parent_label() {
        ...
        let a = photos.join("a.jpg");
        let b = photos.join("b.jpeg");
