...
#[derive(Clone, Debug)]
pub struct Toolchain { pub(crate) encoder: PathBuf, pub(crate) decoder: PathBuf }
...
impl Toolchain {
    ...
    pub fn from_directory(directory: &Path) -> Result<Self> {
        ...
        if !encoder.is_file() || !decoder.is_file() {
            return Err(Error::Invalid("В папке нужны оба файла: cjxl и djxl (в Windows — .exe)".into()));
        }
    ...
    pub fn probe(&self, control: &Control) -> Result<ToolInfo> {
        ...
        let help = process::run(&self.decoder, &["--help".into(), "-v".into()], control, timeout)?;
        ...
        let encoder_help = process::run(&self.encoder, &["--help".into(), "-v".into(), "-v".into()], control, timeout)?;
        ...
        let first_line = |text: String| text.lines().find(|l| !l.trim().is_empty()).unwrap_or("unknown").to_owned();
        Ok(ToolInfo {
            directory: self.encoder.parent().unwrap_or(Path::new("")).into(),
            encoder_version: first_line(encoder), decoder_version: first_line(decoder),
        })
    }
}
