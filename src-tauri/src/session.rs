...
pub struct AppState { pub session: Arc<Session> }
...
impl Session {
    ...
    pub fn begin(self: &Arc<Self>) -> Result<Operation, String> {
        let mut current = self.control.lock().map_err(|_| "Сбой состояния приложения")?;
        ...
        *current = control.clone();
        Ok(Operation { session: self.clone(), control })
    }
}
