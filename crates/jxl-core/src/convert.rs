use crate::{
    files, process, Control, ConversionMode, Error, ItemResult, ItemStatus, Options, Progress,
    Result, SourceFile, Stage, Toolchain,
};
use std::{
    fs,
    path::Path,
...
pub(crate) fn convert_one(
    ...
    let work = || -> Result<Outcome> {
        ...
        match fs::symlink_metadata(&target) {
            Ok(_) => {
                return Ok(Outcome {
                    ...
                    output_bytes: None,
                    sha256: None,
                    message: Some("Файл уже существует; он не перезаписан и не проверялся".into()),
                })
            }
        ...
        let snapshot = temp
            .path()
            .join(if options.mode == ConversionMode::JpegToJxl {
                "input.jpg"
            } else {
                "input.jxl"
            });
        ...
        if options.mode == ConversionMode::JxlToJpeg {
            ...
            process::run(
                ...
                &[
                    ...
                    stage.as_os_str().into(),
                    "--pixels_to_jpeg".into(),
                    format!("--num_threads={threads}").into(),
                ],
                control,
                CODEC_TIMEOUT,
            )?;
            ...
            if size < 3 {
                return Err(Error::Invalid(
                    "Декодер создал пустой или повреждённый JPEG".into(),
                ));
            }
            use std::io::Read;
            ...
            if signature != [0xff, 0xd8, 0xff] {
                return Err(Error::Invalid(
                    "Результат не содержит сигнатуру JPEG".into(),
                ));
            }
            files::ensure_stamp(source)?;
            if !files::equal_files(&source.source, &snapshot, control)? {
                return Err(Error::SourceChanged);
            }
            files::sync_stage(&stage)?;
            if options.preserve_metadata {
                files::preserve_metadata(source, &stage)?;
            }
            control.check()?;
        ...
        process::run(
            ...
            &[
                snapshot.as_os_str().into(),
                stage.as_os_str().into(),
                "--lossless_jpeg=1".into(),
                "--allow_jpeg_reconstruction=1".into(),
                format!("--effort={}", options.effort).into(),
                format!("--num_threads={threads}").into(),
            ],
            control,
            CODEC_TIMEOUT,
        ...
        process::run(
            ...
            &[
                stage.as_os_str().into(),
                reconstructed.as_os_str().into(),
                "--reconstruct_jpeg".into(),
                format!("--num_threads={threads}").into(),
            ],
            control,
            CODEC_TIMEOUT,
        )?;
        if !files::equal_files(&snapshot, &reconstructed, control)? {
        ...
        let size = fs::metadata(&stage)?.len();
        if size == 0 {
            return Err(Error::Invalid("Кодек создал пустой файл".into()));
        }
        if options.skip_larger && size >= source.size {
            return Ok(Outcome {
                status: ItemStatus::NotSmaller,
                output_bytes: Some(size),
                sha256: Some(hash),
                message: Some("Размер не уменьшился; JXL не сохранён".into()),
            });
        }
        files::sync_stage(&stage)?;
        if options.preserve_metadata {
...
mod tests {
    ...
    fn successful_process_with_wrong_reconstruction_is_rejected() {
        ...
        let options = Options {
            ...
            effort: 7,
            performance: crate::Performance::Quiet,
