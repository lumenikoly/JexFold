...
pub(crate) fn run(
    ...
    let out = child
        ...
        .stdout
        .take()
        .ok_or_else(|| Error::Invalid("Отсутствует stdout кодека".into()))?;
    let err = child
        .0
        .stderr
        .take()
        .ok_or_else(|| Error::Invalid("Отсутствует stderr кодека".into()))?;
    ...
    thread::scope(|scope| {
        ...
        let output = stdout
            .join()
            .map_err(|_| Error::Invalid("Сбой чтения stdout".into()))??;
        let errors = stderr
            .join()
            .map_err(|_| Error::Invalid("Сбой чтения stderr".into()))??;
        let status = status?;
