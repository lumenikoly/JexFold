...
pub async fn scan_sources(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        *session.scan.lock().map_err(|_| "Сбой состояния очереди")? = None;
        let result =
            jxl_core::scan_sources(&paths, mode, &_operation.control).map_err(|e| e.to_string())?;
    ...
    if let Some(directory) = directory {
        ...
        if let Ok(executable) = std::env::current_exe() {
            if let Some(directory) = executable.parent() {
                candidates.push(directory.join("codecs"));
            }
        }
        if let Ok(path) = app.path().resolve("codecs", BaseDirectory::Resource) {
...
pub async fn convert(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let scan = session
            .scan
            .lock()
            .map_err(|_| "Сбой очереди")?
            .clone()
            .ok_or("Сначала выберите JPEG")?;
        let tools = session
            .tools
            .lock()
            .map_err(|_| "Сбой кодека")?
            .clone()
            .ok_or("Кодек не настроен")?;
        jxl_core::run_batch(&scan, &options, &tools, &_operation.control, |event| {
            if on_event.send(event).is_err() {
                _operation.control.cancel();
            }
        })
        .map_err(|e| e.to_string())
    })
...
pub fn cancel(state: State<'_, AppState>) -> Result<(), String> {
    let control = state
        ...
        .control
        .lock()
        .map_err(|_| "Сбой состояния операции")?;
    if !state.session.is_busy() {
        return Err("Операция ещё не запущена или уже завершена".into());
    }
    control.cancel();
    Ok(())
}
...
pub fn set_paused(paused: bool, state: State<'_, AppState>) -> Result<(), String> {
    let control = state
        ...
        .map_err(|_| "Сбой состояния операции")?;
    if !state.session.is_busy() {
        return Err("Нет выполняющейся операции".into());
    }
