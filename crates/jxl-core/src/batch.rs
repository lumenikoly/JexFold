...
use crate::{convert::convert_one, files, ConversionMode, Control, Error, ItemStatus, Options, Progress, Result, ScanResult, Summary, Toolchain};
...
pub fn run_batch(
    ...
    control.check()?;
    if options.mode != scan.mode { return Err(Error::Invalid("Режим изменился после выбора файлов. Выберите источники заново.".into())); }
    if options.mode == ConversionMode::JpegToJxl && !(3..=9).contains(&options.effort) { return Err(Error::Invalid("Усилие сжатия должно быть от 3 до 9".into())); }
    ...
    let mut summary = Summary { total: scan.files.len(), ..Summary::default() };
    ...
    if worker_panic { return Err(Error::Invalid("Сбой рабочего потока. Уже созданные файлы сохранены.".into())); }
    Ok(summary)
}
