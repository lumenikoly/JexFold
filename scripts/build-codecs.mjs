...
try {
  ...
  if (!fs.existsSync(source)) {
    run('git', ['clone', '--depth', '1', '--branch', VERSION, 'https://github.com/libjxl/libjxl.git', source]);
  }
  const commit = capture('git', ['rev-parse', 'HEAD'], source).trim();
  if (!commit.startsWith(EXPECTED_COMMIT_PREFIX)) throw new Error(`Unexpected libjxl checkout: ${commit}. Expected ${VERSION}/${EXPECTED_COMMIT_PREFIX}.`);
  ...
  run('git', ['submodule', 'update', '--init', '--depth', '1',
    'third_party/brotli', 'third_party/highway', 'third_party/skcms', 'third_party/libjpeg-turbo'], { cwd: source });
  ...
  const jpegBuild = path.join(root, '.tools', `jpeg-${VERSION}-${process.platform}-${process.arch}`);
  const platformOptions = process.platform === 'darwin'
    ? ['-DCMAKE_OSX_DEPLOYMENT_TARGET=11.0', `-DCMAKE_OSX_ARCHITECTURES=${process.arch === 'arm64' ? 'arm64' : 'x86_64'}`]
    : [];
  const jobs = String(Math.min(4, os.availableParallelism()));
  run('cmake', ['-S', jpegSource, '-B', jpegBuild,
    '-DCMAKE_BUILD_TYPE=Release', '-DCMAKE_POLICY_VERSION_MINIMUM=3.5', '-DENABLE_SHARED=OFF', '-DENABLE_STATIC=ON',
    '-DWITH_TURBOJPEG=OFF', '-DWITH_SIMD=OFF', '-DCMAKE_POSITION_INDEPENDENT_CODE=ON',
    ...(process.platform === 'win32' ? ['-DWITH_CRT_DLL=OFF'] : []), ...platformOptions]);
  run('cmake', ['--build', jpegBuild, '--config', 'Release', '--target', 'jpeg-static', '--parallel', jobs]);
  const jpegLib = [
    path.join(jpegBuild, 'Release', 'jpeg-static.lib'), path.join(jpegBuild, 'jpeg-static.lib'),
    path.join(jpegBuild, 'libjpeg.a'), path.join(jpegBuild, 'Release', 'libjpeg.a'),
  ...
  const config = [
    '-S', source, '-B', build,
    '-DCMAKE_BUILD_TYPE=Release', '-DCMAKE_POLICY_VERSION_MINIMUM=3.5', '-DBUILD_TESTING=OFF', '-DBUILD_SHARED_LIBS=OFF',
    '-DJPEGXL_ENABLE_TOOLS=ON', '-DJPEGXL_ENABLE_TRANSCODE_JPEG=ON',
    '-DJPEGXL_ENABLE_BOXES=ON', '-DJPEGXL_ENABLE_SKCMS=ON',
    '-DJPEGXL_ENABLE_DEVTOOLS=OFF', '-DJPEGXL_ENABLE_BENCHMARK=OFF',
    '-DJPEGXL_ENABLE_EXAMPLES=OFF', '-DJPEGXL_ENABLE_FUZZERS=OFF',
    ...
    '-DCMAKE_DISABLE_FIND_PACKAGE_PNG=ON', '-DCMAKE_DISABLE_FIND_PACKAGE_GIF=ON',
    '-DCMAKE_DISABLE_FIND_PACKAGE_JPEG=OFF', '-DJPEGXL_BUNDLE_LIBPNG=OFF',
  ...
  run('cmake', config);
  run('cmake', ['--build', build, '--config', 'Release', '--target', 'cjxl', 'djxl',
    '--parallel', jobs]);
  fs.mkdirSync(codecsDir, { recursive: true });
  for (const name of ['cjxl', 'djxl']) {
    const candidates = [path.join(build, 'tools', 'Release', name + extension), path.join(build, 'tools', name + extension)];
  ...
  for (const entry of fs.readdirSync(build)) {
    if (entry.startsWith('LICENSE.')) fs.copyFileSync(path.join(build, entry), path.join(licenseDir, entry));
  }
  const notices = [
    ['LICENSE', 'libjxl-LICENSE'], ['PATENTS', 'libjxl-PATENTS'],
  ...
  for (const [from, to] of notices) {
    if (fs.existsSync(path.join(source, from))) fs.copyFileSync(path.join(source, from), path.join(licenseDir, to));
  }
  const manifest = {
    libjxlVersion: VERSION, commit, platform: process.platform, arch: process.arch,
  ...
  fs.writeFileSync(path.join(codecsDir, 'build-info.json'), JSON.stringify(manifest, null, 2) + '\n');
  ...
  console.error('See README.md → Системные зависимости. On Windows use a VS Developer terminal with C++ tools.');
  process.exitCode = 1;
}
