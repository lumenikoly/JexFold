...
fn reconstructs_baseline_and_progressive_jpeg_with_metadata() {
    ...
    fs::create_dir_all(&input).unwrap(); fs::create_dir_all(&output).unwrap();
    ...
    assert!(!fs::read_dir(output.join("photos")).unwrap().any(|f| f.unwrap().file_name().to_string_lossy().starts_with(".jxl-archiver-")));
}
...
fn rejects_nested_output_before_executing_codec() {
    ...
    let input = t.path().join("photos"); let output = input.join("converted");
    ...
    let bin = t.path().join("tools"); fs::create_dir(&bin).unwrap();
