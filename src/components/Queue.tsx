...
export function Queue({ scan, rows, finished, mode }: { scan: ScanResult | null; rows: Map<number, RowState>; finished: boolean; mode: ConversionMode }) {
  ...
  return <>
    <div className="table-scroll"><table>
      <thead><tr><th>{t('queue.file')}</th><th>{mode === 'jpegToJxl' ? 'JPEG' : 'JXL'}</th><th>{mode === 'jpegToJxl' ? 'JXL' : 'JPEG'}</th><th>{t('queue.result')}</th><th aria-label={t('progress.saved')}></th></tr></thead>
      <tbody>{visible.map(file => { const state = rows.get(file.id); const result = state?.result; const saving = result?.outputBytes != null && mode === 'jpegToJxl' ? savingPercent(file.size, result.outputBytes) : null; return <tr key={file.id}>
        ...
        <td><span className={`status ${state?.status ?? 'pending'}`}><span className="status-dot" />{state ? t(`status.${state.status}`) : t(finished ? 'status.notProcessed' : 'status.queued')}</span></td>
        <td className={`saving-number ${saving != null && saving > 0 ? 'positive' : ''}`}>{saving == null ? '—' : intl.formatNumber(-saving / 100, { style: 'percent', maximumFractionDigits: 1 })}</td>
      </tr>; })}</tbody>
    </table></div>
