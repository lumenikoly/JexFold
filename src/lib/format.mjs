...
export function formatBytes(bytes, locale = 'en') {
  ...
  const units = locale.toLowerCase().startsWith('ru') ? ['Б', 'КиБ', 'МиБ', 'ГиБ', 'ТиБ'] : ['B', 'KiB', 'MiB', 'GiB', 'TiB'];
  let index = 0;
...
export function duration(milliseconds, locale = 'en') {
  ...
  return seconds < 60 ? `${seconds} ${secondUnit}` : `${Math.floor(seconds / 60)} ${minuteUnit} ${seconds % 60} ${secondUnit}`;
}
