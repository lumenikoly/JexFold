...
export function run(command, args = [], options = {}) {
  ...
  const result = spawnSync(executable, args, { cwd: root, stdio: 'inherit', shell: false, ...options });
...
export function capture(command, args, cwd = root) {
  const executable = resolveCommand(command);
  console.log(`> ${executable} ${args.join(' ')}`);
  const result = spawnSync(executable, args, { cwd, encoding: 'utf8', shell: false, timeout: 120_000, maxBuffer: 2 * 1024 * 1024 });
